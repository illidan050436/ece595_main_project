...
var step_para; // explaination paragraph
var speed = 10;
var stopped = 1;
var started = 0;
...
function button1_event(){
  //step_para.innerHTML = "Changed by button1_event";
...
function button2_event(){
  //step_para.innerHTML = "Changed by button2_event";
  for (var i = 0;i<5;i++)
   for (var j = 0;j<5;j++){
...
function updateMsg(i,j){
  ...
  if (b[j].lowestid == undefined || b[j].lowestid > (i+1)){
    b[j]["lowestid"] = i+1;
  }
...
function drawTable(){
  ...
  if (Date.now()-statetime > speed*714 ){
...
function test_start(){
  ...
  step_para = document.getElementById("step_para");
  // initialization
  init();
  stopped = 0; 
  //AnimationId=window.requestAnimationFrame(drawScene);
